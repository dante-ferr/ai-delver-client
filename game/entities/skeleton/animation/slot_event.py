...
class SlotEvent(AnimationEvent):
    ...
    def __init__(self, slot: Slot, event_type: SlotEventType, event_sequence: dict, event_index=0, start_duration=0):
    ...
    def update(self, frame_step: float, new_event_callback: Callable):
        return super().update(frame_step, lambda event_sequence, event_index, start_duration: 
            new_event_callback(self.slot, self.event_type, event_sequence, event_index, start_duration)
        )
